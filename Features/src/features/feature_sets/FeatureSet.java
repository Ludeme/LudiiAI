...
import features.instances.OneOfMustEmpty;
import features.instances.OneOfMustWhat;
import features.instances.OneOfMustWho;
import game.Game;
...
public class FeatureSet extends BaseFeatureSet 
{
	...
	protected FVector featureInitWeights = null;
	...
	)
	{
		...
		if (weights == null)
			featureInitWeights = null;
		else
			featureInitWeights = new FVector(weights);
		...
		// feature generation
		final Context featureGenContext = new Context(game, new Trial(game));
...
//		System.out.println("---");
	...
	)
	{
		...
		// but more efficient		
		final boolean[] featuresActive = new boolean[features.length];
...
//		}
	...
	)
	{
		...
		final List<Feature> activeFeatures = new ArrayList<Feature>(activeFeatureIndices.size());
		
		final TIntIterator it = activeFeatureIndices.iterator();
		while (it.hasNext())
		{
			activeFeatures.add(features[it.next()]);
		}
	 ...
	 * positions.
	 */
	public Footprint generateFootprint
	(
		final State state, 
		final int from, 
		final int to,
...
//			System.out.println("to = " + to);
	...
	)
	{
		...
		while (!allPairs.isEmpty())
		{
			...
			final FeatureSet newFeatureSet = createExpandedFeatureSet(game, pair.a, pair.b);
	...
	{
		final Feature combinedFeature = 
				Feature.combineFeatures
				(
					targetGame, 
					firstFeatureInstance, 
					secondFeatureInstance
				);
		
		boolean featureAlreadyExists = false;
		for (final Feature oldFeature : features)
		{
			...
			{
				allowedRotations = Walk.allGameRotations(game);
			}
			
			for (int i = 0; i < allowedRotations.size(); ++i)
			{
				final Feature rotatedCopy = 
						combinedFeature.rotatedCopy(allowedRotations.getQuick(i));
		...
		if (!featureAlreadyExists)
		{
			...
			final List<Feature> newFeatureList = new ArrayList<Feature>(features.length + 1);
...
//		System.out.println("---");
	...
	private static class FastFeaturesNode
	{
		...
		public static FastFeaturesNode thresholdedNode(final FastFeaturesNode other, final FVector weights)
		{
			...
			{
				if (weights == null || Math.abs(weights.get(activeFeature)) >= FEATURE_WEIGHT_THRESHOLD)
