...
public class BipartiteGraphFeatureInstanceSet
{
	 ...
	 * @return A PropSet representation of this feature set
	 */
	public PropSet toClauseFeatureSet(final int numFeatures, final BitSet thresholdedFeatures)
	{
		...
		for (int i = filteredInstanceNodes.size() - 1; i >= 0; --i)
		{
			...
			final int featureIdx = node.instance.feature().featureSetIndex();
		...
		for (int i = filteredInstanceNodes.size() - 1; i >= 0; --i)
		{
			...
			final int featureIdx = instanceNode.instance.feature().featureSetIndex();
			...
			for (int k = 0; k < filteredInstanceNodes.size(); ++k)
			{
				if (i != k)
				{
					...
					if (other.instance.feature().featureSetIndex() == featureIdx)
					{
		...
		{
			final int featureIdx = filteredInstanceNodes.get(i).instance.feature().featureSetIndex();
		...
		for (int i = 0; i < propositionNodesList.size(); ++i)
		{
			...
			for (int j = 0; j < propositionNodesList.size(); ++j)
			{
				...
				if (propI.provesIfTrue(propJ))
					proveIfTrue[i].set(j);
				else if (propI.disprovesIfTrue(propJ))
					disproveIfTrue[i].set(j);
				
				if (propI.provesIfFalse(propJ))
					proveIfFalse[i].set(j);
				else if (propI.disprovesIfFalse(propJ))
...
//			System.out.println();
		...
		for (int j = 0; j < sortableFeatureInstances.size(); ++j)
		{
			...
			instancesPerFeature[sortedFeatureInstances[j].feature().featureSetIndex()].set(j);
		}
		...
		for (int j = 0; j < propositions.size(); ++j)
		{
			...
			for (int k = j + 1; k < propositions.size(); ++k)
			{
				...
				if (propJ.provesIfTrue(propK))
					provesIfTruePerProp[j].set(k);
				else if (propJ.disprovesIfTrue(propK))
					disprovesIfTruePerProp[j].set(k);
				
				if (propJ.provesIfFalse(propK))
					provesIfFalsePerProp[j].set(k);
				else if (propJ.disprovesIfFalse(propK))
