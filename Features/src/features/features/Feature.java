...
/**
 * Features contain a Pattern that can be matched to parts of the board, and
...
public abstract class Feature
{
	 ...
	 */
	public abstract Feature rotatedCopy(final float rotation);
	 ...
	 */
	public abstract boolean generalises(final Feature other);
							...
							for (int testSiteIdx = 0; testSiteIdx < testSites.size(); ++testSiteIdx)
							{
								...
								else if (type == ElementType.Friend) // entry in "who" BitSetS must (not) equal the player
								{
									if (testSite >= 0)
									{
										...
										if (newInstance.addTest(container, BitSetTypes.Who, testSite, !element.not(),
												player))
										{
											replaceNewInstances.add(newInstance);
										}
								...
								else if (type == ElementType.Enemy)
								{
									...
									else if (testSite >= 0)
									{
										if (game.players().count() == 2)
										{
											...
											// enemy directly
											final FeatureInstance newInstance = new FeatureInstance(instance);
											if (newInstance.addTest(container, BitSetTypes.Who, testSite, true,
													player == 1 ? 2 : 1))
											{
												replaceNewInstances.add(newInstance);
											}
										...
										else
										{
											...
											{
												if (newInstance.addTest(container, BitSetTypes.Who, testSite, false,
														player))
												{
													replaceNewInstances.add(newInstance);
												}
								...
								else if (type == ElementType.Item)
								{
									...
									{
										final FeatureInstance newInstance = new FeatureInstance(instance);
										if (newInstance.addTest(container, BitSetTypes.What, testSite, !element.not(),
												element.itemIndex()))
										{
											replaceNewInstances.add(newInstance);
										}
	...
	public static Feature combineFeatures(final Game game, final FeatureInstance a, final FeatureInstance b)
	{
		...
		// create modified copies of Patterns
		...
		else if (featureA instanceof RelativeFeature && featureB instanceof RelativeFeature)
		{
			...
			final RelativeFeature newFeature = new RelativeFeature(newPattern, newToPosition, newFromPosition);
	...
	public static List<Feature> deduplicate(final List<Feature> features)
	{
		...
		final List<Feature> deduplicated = new ArrayList<Feature>(features.size());
